...
sub print_nasm_header($$$) {
  my($outfh, $cpulevel, $data_alignment) = @_;
...
%ifidn __OUTPUT_FORMAT__, elf  ; Make it work without elf.inc.nasm.
  ...
  %endm
%else
  %include 'elf.inc.nasm'  ; To make `nasm -f bin' produce an ELF executable program.
...
my $srcfmt = detect_assembly_format($srcfh);
die "fatal: assembly source file format not recognized: $srcfn\n" if !defined($srcfmt);
